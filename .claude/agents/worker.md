...
```
TodoWrite([
  { content: "対象ファイルを読み込む", status: "completed", activeForm: "Reading target files" },
  { content: "実装を行う", status: "in_progress", activeForm: "Implementing changes" },
  { content: "テストを実行する", status: "pending", activeForm: "Running tests" },
  { content: "完了報告を作成する", status: "pending", activeForm: "Creating completion report" }
])
```
